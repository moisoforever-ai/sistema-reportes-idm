...
# Tabla dinámica nativa de Excel (sesión 22, a pedido del cliente) — librería
# de terceros, experimental, fijada a un commit específico en
# requirements.txt. install() agrega el método add_pivot_table() a
# Worksheet; si la librería no está disponible por algún motivo, el resto
# del sistema sigue funcionando igual (esa tabla específica simplemente no
# se genera, ver el try/except donde se usa).
try:
    from openpyxl_pivots import install as _install_openpyxl_pivots
    _install_openpyxl_pivots()
except Exception:
    pass
...
@app.route('/admin/exportar_marcas_kobo', methods=['GET'])
def exportar_marcas_kobo():
    """
    Genera marcas_catalogo.csv que usa el selector de MARCA del formulario
    de Kobo (select_one_from_file) — mismo criterio que
    exportar_catalogo_kobo, pero para marcas en vez de productos.

    Se deduplican variantes de mayúsculas/minúsculas de la misma marca
    (ej. "Daewoo"/"DAEWOO" en la base real de Daka son la misma marca
    escrita distinto) — se conserva la variante más usada en los datos
    reales, para no forzar una marca que normalmente va en mayúsculas
    (ej. "LG", "JBL") a un Title Case que se vería raro. También se
    descartan valores basura de carga (ej. la palabra suelta "Marca").

    Uso: igual que el catálogo de productos — descargar y subir en la
    pestaña "Media" del proyecto de Kobo (nombre exacto:
    marcas_catalogo.csv), después Redeploy.
    """
    if not is_logged_in():
        return redirect(url_for('login_page'))
    if not is_admin():
    ...
    VALORES_BASURA = {'marca', 'n/a', 'na', 's/m', 'sin marca'}
    ...
    for empresa_code, gid_base in empresas_gid:
        try:
            ...
            app.logger.warning(f"Error al exportar marcas Kobo para {empresa_code}: {e}")
            errores.append(empresa_code)
            continue

        marcas_crudas = df_maestro['MARCA'].dropna().astype(str).str.strip()
        marcas_crudas = marcas_crudas[marcas_crudas.str.lower().apply(lambda x: x not in VALORES_BASURA and x != 'nan' and x != '')]

        # Agrupar por minúscula (para no duplicar "Daewoo"/"DAEWOO"), y
        # quedarse con la variante de mayúsculas más usada en los datos reales.
        conteo = marcas_crudas.value_counts()
        por_minuscula = {}
        for valor, cnt in conteo.items():
            clave = valor.lower()
            if clave not in por_minuscula or cnt > por_minuscula[clave][1]:
                por_minuscula[clave] = (valor, cnt)

        for valor_canonico, _ in sorted(por_minuscula.values(), key=lambda x: x[0].lower()):
            filas.append({'name': valor_canonico, 'label': valor_canonico, 'empresa': empresa_code})
    ...
    respuesta = send_file(
        ...
        mimetype="text/csv",
        as_attachment=True,
        download_name="marcas_catalogo.csv"
    )
    if errores:
        respuesta.headers['X-Empresas-Con-Error'] = ','.join(errores)
    return respuesta
...
def generate_report():
    ...
    try:
        ...
        for idx, row in df_filtered.iterrows():
            ...
            # FIX (sesión 22): "Producto"/"Marca" pueden venir vacíos (NaN) en
            # algunas filas reales — se pasan a texto vacío en vez de dejar
            # un NaN/float suelto, porque una columna con tipos mezclados
            # (texto + NaN) rompe la tabla dinámica nativa más abajo.
            producto_campo_val = '' if pd.isna(row['Producto']) else row['Producto']
            marca_campo_val = '' if pd.isna(row['Marca']) else row['Marca']
            ws2.cell(row=raw_row_idx, column=4, value=producto_campo_val).alignment = Alignment(horizontal="center", vertical="center")
            ws2.cell(row=raw_row_idx, column=5, value=marca_campo_val).alignment = Alignment(horizontal="center", vertical="center")
            ...
            # FIX (sesión 22): "Factura" tiene NaN mezclado con números en
            # datos históricos (de antes de que el campo fuera obligatorio
            # en Kobo) — se usa 0 como valor consistente en vez de dejar la
            # columna con tipos mezclados, por la misma razón que
            # Producto/Marca más arriba.
            ...
            visitas_val = 0 if pd.isna(row['Visitas']) else row['Visitas']
            ws2.cell(row=raw_row_idx, column=13, value=visitas_val).alignment = Alignment(horizontal="center", vertical="center")
        ...
        # librería de terceros "openpyxl-pivots" (experimental, fijada a un
        # commit específico en requirements.txt para que no cambie sola).
        ...
        # "Datos Detallados". Se ordena una vez con un clic en Excel
        # (clic derecho sobre la columna de valores → Ordenar → De mayor a
        # menor) y Excel recuerda ese orden en las próximas actualizaciones.
        try:
            ultima_fila_dd = raw_row_idx - 1
            rango_fuente = f"'Datos Detallados'!A1:P{ultima_fila_dd}"
